...
import { RemoveFile, SaveFile } from '@/api/file/index'
...
// 展示移动文件界面
function ShowSaveFileTable(fileId:string) {
    showTable.value = true
    SaveFileId.value = fileId
...
async function getFileFolderInfo(filefolderId: string) {
  try {
    ...
    ElMessageBox.alert("网络错误,无法获取用户文件夹", "ops!")
  }
}
...
async function confirmRemove() {
  ...
  try {
    await SaveFile({
      fileid: SaveFileId.value,
      filefolder: nowFileFolder.value
    })
    ElMessage({
      message: '保存成功',
      type: 'success',
...
defineExpose({ ShowSaveFileTable });
</script>
...
<template>
    <el-dialog v-model="showTable" width="800">
      <template #header="{ titleId, titleClass }">
        <div class="my-header">
          <h4 :id="titleId" :class="titleClass">选择想要保存到的文件夹</h4>
